...
class DenunciaCreate(BaseModel):
    categoria: str                        # corrupcion / acoso / fraude / otro
    descripcion: str
...
class DenunciaUpdate(BaseModel):
    estado: Optional[str] = None          # recibida / en_investigacion / cerrada
...
class MensajeSeguimiento(BaseModel):
    mensaje: str
    autor: str = "denunciante"            # denunciante / compliance


# ── Helpers ──────────────────────────────────────────────────────────────────
def _codigo_corto() -> str:
    """UUID corto de 8 caracteres, mayúsculas."""
...
def _generar_alertas_si_procede(conn, denuncia_id: int, denuncia: dict):
    """Crea alertas automáticas si los plazos están vencidos o próximos."""
    ...
    # Acuse de recibo vencido (>7 días y estado todavía 'recibida')
    ...
    # Caso crítico: prioridad alta y en investigación > 30 días
...
def crear_denuncia(body: DenunciaCreate):
    """
    Crea una denuncia y devuelve el código de seguimiento único.
    No requiere autenticación. Si es_anonima=True, datos_contacto no se almacena.
    Plazos calculados según Ley 2/2023: acuse +7 días, cierre +90 días.
    """
...
def consultar_seguimiento(codigo: str):
    ...
    try:
        ...
        # Nunca exponer datos_contacto al consultante (ya los conoce)
...
def agregar_mensaje_seguimiento(codigo: str, body: MensajeSeguimiento):
    """
    El denunciante (o compliance) agrega información al caso sin revelar identidad.
    autor debe ser 'denunciante' o 'compliance'.
    """
...
# ── Endpoints de gestión (panel compliance) ──────────────────────────────────

@router.get("/denuncias", summary="Listar todas las denuncias (panel)")
def listar_denuncias(
    estado: Optional[str] = Query(None, description="recibida / en_investigacion / cerrada"),
    ...
    """Panel de gestión: lista denuncias con filtros opcionales. Sin auth por ahora."""
    filters = []
    params = []
    ...
    if area:
        filters.append("area_relacionada = ?")
        params.append(area)
    ...
    try:
        ...
        total = conn.execute(
            f"SELECT COUNT(*) FROM denuncias {where}",
            params[:-2]
        ).fetchone()[0]
        ...
        for row in rows:
            d = _row_to_dict(row)
            # Generar alertas al listar (lazy evaluation)
...
@router.patch("/denuncias/{id}", summary="Actualizar estado/prioridad (panel)")
def actualizar_denuncia(id: int, body: DenunciaUpdate):
    """Cambia estado, prioridad o área relacionada de una denuncia."""
    conn = get_conn()
    try:
        ...
        # Si se cierra, agregar actualización automática
...
@router.get("/denuncias/alertas", summary="Alertas activas canal denuncias (panel)")
def listar_alertas(solo_no_leidas: bool = Query(True)):
    """
    Devuelve alertas activas: plazos vencidos, casos críticos, plazos próximos.
    Incluye datos de la denuncia relacionada para contexto inmediato.
    """
    conn = get_conn()
    try:
...
@router.patch("/denuncias/alertas/{id}/leer", summary="Marcar alerta como leída")
def marcar_alerta_leida(id: int):
    conn = get_conn()
    try:
...
@router.get("/denuncias/resumen/por-area", summary="Resumen por área (cruce con Mapa de Calor)")
def resumen_por_area():
    """
    Agrega denuncias por área_relacionada y estado.
    Pensado para cruzar con el Mapa de Calor de riesgos existente.
    """
    conn = get_conn()
    try:
        rows = conn.execute(
