...
def _pin_seguro(nombre_env: str) -> str:
    """
    Devuelve el PIN configurado por variable de entorno.
    IMPORTANTE: nunca usa un valor por defecto fijo/adivinable (ej. "0000").
    Si la variable no está seteada, genera un PIN aleatorio de 6 dígitos
    para esta ejecución del proceso y lo deja en el log del servidor una
    sola vez al arrancar, para que el operador lo pueda usar hasta que
    configure la variable de entorno real en Railway.
    """
    valor = os.getenv(nombre_env)
    if valor:
        return valor
    generado = "".join(secrets.choice("0123456789") for _ in range(6))
    log.warning(
        "%s no está configurado en las variables de entorno de Railway. "
        "Se generó un PIN temporal para esta ejecución del servidor: %s "
        "— este PIN cambia en cada redeploy/restart. Configurá %s en Railway "
        "(Settings → Variables) con un valor fijo y fuerte para producción.",
        nombre_env, generado, nombre_env,
    )
    return generado


PIN_CONSULTOR   = _pin_seguro("PIN_CONSULTOR")
PIN_COMPLIANCE  = _pin_seguro("PIN_COMPLIANCE")
...
# ── Base de datos ────────────────────────────────────────────────────────────
# DENUNCIAS_DB_PATH permite apuntar a un Volume persistente de Railway
# (ver readme.md — sección Persistencia). Si no está seteada, usa la ruta
# local por defecto (se pierde en cada redeploy si no hay volumen montado).
DB_PATH = os.getenv("DENUNCIAS_DB_PATH") or os.path.join(
    os.path.dirname(__file__), "..", "data", "denuncias.db"
)
